          ...
          <header className="col s12 m4 project-info">
            ...
            <p className="home">
              ...
              {project.links && project.links.map(link => {
                ...
                return (<>
                  <br />
                  <a href={l} key={l} target="_blank" rel="noopener noreferrer">{l}</a>
                </>)
              })}
            </p>
          ...
          {project.tags && (
            <ul>
              {project.tags.map(tag => (
                <li key={tag}><a className="waves-effect waves-light btn btn-flat modal-trigger chip" href={`#modal${project.pages}`} data-tag={tag}>{tag}</a></li>
              ))}
            </ul>
          )}
        </footer>
