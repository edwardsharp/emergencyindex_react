...
export const projects:iProject[] = [
  {
    ...
    first_performed: `${faker.date.month()} ${Math.max(1, faker.random.number(28))}, 2011`,
    ...
    collaborators: Array.from({length: faker.random.number(6)}, () => `${faker.name.firstName()} ${faker.name.lastName()}`),
    ...
    pages: `002-003`,
    needs_review: true,
    description: `${faker.hacker.phrase()} ${faker.hacker.phrase()}\n${faker.lorem.paragraphs(Math.max(4, faker.random.number(10)))}`
  }
]